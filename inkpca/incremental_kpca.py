...
class IncrKPCA(object):
    ...
    def __init__(self, X, m0, mmax=None, kernel=kernel_error, adjust=False,
                     nystrom=False, eps=1E-9, r=None, maxiter=500):
    ...
    def get_idx_array(self):
        """
        Getter for the indexes used each time
        """
        ...
        if self.nystrom:
            if not rc:
                self.L_nys, self.U_nys = nystrom_approximation(
                        self.L, self.U, self.K_nm, self.eps, self.r)
    ...
    def init_K_nm(self):
        sums = np.expand_dims(self.K_nm.sum(1),0).T / self.m0
        total_sum = sums.sum() / self.n
        self.K_nm = self.K_nm - sums - sums[self.idx[:self.m0]].T + total_sum
        self.sums = sums
        self.total_sum = total_sum

    def update_K_nm(self):
        """
        Update K_nm for one iteration. If mean is not adjusted, just adding
        an additional column. If mean is adjusted, also adjust to the new
        approximation for the mean of K.
        """
        i = self.i
        K_ni = kernel_matrix(self.X, self.kernel, range(self.n), [self.idx[i]])
        if self.adjust:
            self.K_nm = self.K_nm + self.sums + self.sums[self.idx[:i]].T - self.total_sum
            self.sums = self.sums * i / (i+1) + K_ni / (i+1)
            self.total_sum = self.sums.sum() / self.n
            self.K_nm = self.K_nm - self.sums - self.sums[self.idx[:i]].T + self.total_sum
            K_ni = K_ni - self.sums - self.sums[self.idx[i],0] + self.total_sum
...
def nystrom_approximation(L, U, K_nm, eps=1e-9, r=None):
    """
    ...
    eps : float
        epsilon used for numerical regularization
    r : None, int
        If not None, the number of top eigenpairs to keep
