...
def expand_eigensystem(L, U, e):
    """
    Expand eigensystem with new eigenpair
    """
    m = L.shape[0]
    L = np.r_[L, [e]]
...
def update_eigensystem(L, U, v, sigma):
    """
    Perform rank one update to eigensystem. Requires the eigenpairs to
    be ordered. Assumes the ordering of the eigenvalues remains the same
    after the update
...
def rank_one_eigenvalues(L, z, sigma, eps=1E-12):
    ...
    for i in np.arange(n):
        a = bounds[i] + eps
        b = bounds[i+1] - eps
        if omega(a,sigma,z2,L) * omega(b,sigma,z2,L) > 0: # Rank deficient
            L_tilde = None
            break
        else:
